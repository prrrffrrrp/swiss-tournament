...
def playerStandings():
    ...
    ranks = [(row[0], str(row[1]), row[2], row[3]) for row in c.fetchall()]
    conn.close()
    return ranks
...
def swissPairings():
    ...
    for i in range(len(ranking)/2):
        c.execute('select id, name from ranking limit 2 offset (%s)', (offset,))
        pair_1 = c.fetchone()
        pair_2 = c.fetchone()
        pairs.append((pair_1[0],pair_1[1],pair_2[0],pair_2[1]))
        offset += 2
